...
class RawHDF5Dataset(Dataset):
    ...
    def load_annotations(self, ann_file):
        # import ipdb; ipdb.set_trace()
        return [RawHDF5Record(x.strip().split(';')) for x in open(ann_file)]
    ...
    def _load_image(self, bytes_video, image_tmpl, modality, idx, name):
        # import ipdb; ipdb.set_trace()
        if modality in ['RGB', 'RGBDiff']:
            im = Image.open(io.BytesIO(bytes_video[idx]))
            im_arr = np.array(im)
            # import ipdb; ipdb.set_trace()
            # im_arr = cv2.imread(io.BytesIO(bytes_video[idx]))
            return [im_arr]
    ...
    def _sample_indices(self, record):
        ...
        :param record: VideoRawHDF5Record
        :return: list, list
        '''
    ...
    def _get_frames(self, record, image_tmpl, modality, indices, skip_offsets):
        images = list()
        # import ipdb; ipdb.set_trace()
        video = h5py.File(osp.join(self.img_prefix, record.path), 'r')['video']
        for seg_ind in indices:
            p = int(seg_ind) - 1
            # import ipdb; ipdb.set_trace()
            for i, ind in enumerate(range(0, self.old_length, self.new_step)):
                if p + skip_offsets[i] <= record.num_frames:
                    seg_imgs = self._load_image(video,
                        image_tmpl, modality, p + skip_offsets[i], name=record.path)
                else:
                    seg_imgs = self._load_image(video, image_tmpl, modality, p, record.path)
    ...
    def __getitem__(self, idx):
        ...
        # with h5py.File(osp.join('./data', record.path), 'r') as f:
        #     no_frames = f['video'].shape[0]
        # if no_frames > record.num_frames:
        #     no_frames = record.num_frames 
        if self.test_mode:
